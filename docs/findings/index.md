...
| 12 | Large-vocabulary `lm_head` (e.g. Qwen3's ~152K tokens) fails HEF placement as a single monolithic matmul; three independent sharding mechanisms tried (ONNX export, `defuse()`, pre-quantization HN surgery) all hit the same DFC-internal post-fuser bug at shard fan-out ≥3, bracketed against a ~32K-38K width placement ceiling that makes fan-out 2 unusable for a 152K vocabulary | **open, likely unfixable on DFC 5.3.0** | [large-vocab-lm-head-sharding.md](large-vocab-lm-head-sharding.md) |
| 13 | Real 24-layer checkpoints (`Qwen2.5-0.5B-Instruct`) fail `__tbt` compilation with a deterministic context-partition topology error, independent of lm_head shard count/optimization level/context-switch tuning — a separate, downstream DFC limitation at this scale, surfaced only via Finding 12's now-abandoned `defuse()` path | **open, superseded by Finding 12's dead end** | [large-body-multicontext-topology.md](large-body-multicontext-topology.md) |
