...
| 16 | Base-scope (no-cache) generation degenerates to constant token repetition on the same checkpoint (`TinyMistral-248M`) — the float32 HF reference on the identical prompt produces coherent English, ruling out "just a weak checkpoint"; first checkpoint whose lm_head lands on the `N=2` shard-count special case | **open, just discovered** | [tinymistral-base-scope-degenerate.md](tinymistral-base-scope-degenerate.md) |
