...
| 16 | Base-scope (no-cache) generation degenerates to constant token repetition on `TinyMistral-248M` (float32 HF reference on the identical prompt is coherent). Per-shard comparison and a direct surgery-isolation test (forcing the same lm_head-splitting surgery onto the always-coherent TinyStories baseline) both clear the lm_head sharding mechanism — root cause still open, scale/GQA ratio now the leading suspect | **open, sharding surgery cleared as cause** | [tinymistral-base-scope-degenerate.md](tinymistral-base-scope-degenerate.md) |
