...
| 16 | Base-scope (no-cache) generation degenerates to constant token repetition, reproduced on three independent checkpoints (`TinyMistral-248M`, `SmolLM2-135M`, `Felladrin/Llama-160M-Chat-v1`) — the lm_head-splitting surgery and non-power-of-2 GQA ratio were both directly cleared as the cause (the `NREP=1` MHA checkpoint degrades identically); every degraded checkpoint has `NLAYERS≥12`, the only coherent one has `NLAYERS=4` | **open, depth/hidden scale is the leading suspect** | [tinymistral-base-scope-degenerate.md](tinymistral-base-scope-degenerate.md) |
