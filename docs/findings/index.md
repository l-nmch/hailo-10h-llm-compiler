...
| 16 | Base-scope (no-cache) generation degenerates to constant token repetition, reproduced on two independent checkpoints (`TinyMistral-248M`, `SmolLM2-135M`) — the lm_head-splitting surgery was directly cleared as the cause; every degraded checkpoint shares a non-power-of-2 GQA ratio (`NREP` 3/4/7) that every coherent one lacks (`NREP` 1/2) — the strongest lead so far | **open, GQA ratio is the leading suspect** | [tinymistral-base-scope-degenerate.md](tinymistral-base-scope-degenerate.md) |
